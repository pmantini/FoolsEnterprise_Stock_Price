...
class Markov3Class(FEModel):
    ...
    def generate_eval_data(self, column="volume"):
        ...
        dates_samples = dates[:, -self.days_to_eval:total_samples_avail]
        train_samples_actual = values[:, -self.days_to_eval-self.order:total_samples_avail]
    ...
    def do_eval(self):
        ...
        eval = dict()
        class_multiplier = [k for k in reversed([(self.n_classes) ** k for k in range(self.order)])]
        for ind, k in enumerate(dates[0, :-1]):

            day_predict = []
            gt_predict = []
            for s_ind, stock in enumerate(classes[:, ind]):

                initial_state = stock
                predictions = self.predict_next_state(initial_state, transision_matrix[s_ind])

                y = predictions
                gt = classes[s_ind, ind+1]
            ...
            eval[k] = {"pred": day_predict, "gt": gt_predict}
...
class Markov3ClassWeekly(FEModel):
    ...
    def generate_eval_data(self, column = "volume", stats = "max"):
        ...
        train_samples = values[:, -self.weeks_to_eval-1-self.order:total_samples_avail]
        ...
        prices = values[:, -self.weeks_to_eval-1:total_samples_avail]
        dates = dates[:, -self.weeks_to_eval-1:total_samples_avail]
    ...
    #
    #     logging.info("Generating eval data")
    #     eval_data_changes, dates, prices_c = self.generate_eval_data(column="high")
    #
    ...
    #
    #     _, _, _ = self.generate_eval_data(column="close", stats="last")
    #
    #
    #     start_index = []
    #     for k in range(len(daily_dates)):
    #         # print(daily_dates[k], dates[k])
    #         for r in range(len(daily_dates[k])):
    #             if daily_dates[k][r] >= dates[k][0]:
    #                 start_index += [r]
    #                 break
    #         else:
    #             start_index += [len(k)]
    ...
    #     class_multiplier = [k for k in reversed([(self.n_classes) ** k for k in range(self.order)])]
    #
    #     y_true_overall, y_true_overall_l = [], []
    #     y_pred_overall, y_pred_overall_l = [], []
    #     prediction_accuracies_1, prediction_accuracies_3 = [], []
    #     for ind, row in enumerate(classes):
    #
    #         eval_data_for_this_row = row
    #
    #         y_true = eval_data_for_this_row
    #         y_pred = []
    ...
    #
    #         this_original = np.ones(eval_data_changes[ind].shape)
    #         this_original[eval_data_changes[ind] > 0.01] = 2
    #         this_original[eval_data_changes[ind] <= -0.01] = 0
    #
    #         # logging.info(this_original)
    #         prediction_accuracies_1 += [accuracy_score(y_true_order_1[1:], y_pred_order_1)]
    #         logging.info("%s, 1st Order, GT: %s, Pred: %s, acc: %s" % (ind, y_true_order_1[1:], y_pred_order_1, prediction_accuracies_1[-1]))
    #
    #         y_true_overall_l += [y_true_order_1]
    #         y_pred_overall_l += [y_pred_order_1]
    #
    #         y_true_overall += y_true_order_1[1:]
    ...
    #     logging.info("Actual Confusion matrix \n%s" % confusion)
    ...
    #     company_list = self.db.get_list_companies()
    #     eval_iter = 0
    #     for y_true, y_pred in zip(y_true_overall_l, y_pred_overall_l):
    ...
    #         #                 "dates": dates[eval_iter]}
    #         print("-------------")
    #         print(y_true)
    #         print((prices_c[eval_iter, 1:] - prices_c[eval_iter, :-1])/prices_c[eval_iter, :-1])
    #         print(prices_c[eval_iter])
    #         continue
    #         output[comp] = {"accuracy": accuracy_score(y_true[1:], y_pred), "gt": y_true, "pred": y_pred,
    #                         "close": prices_c[eval_iter],
    #                         "high": prices_high[eval_iter, start_index[eval_iter]:],
    #                         "low": prices_low[eval_iter, start_index[eval_iter]:],
    #                         "close_daily": prices_close[eval_iter, start_index[eval_iter]:],
    #                         "dates": dates[eval_iter],
    #                         "daily_dates": daily_dates[eval_iter, start_index[eval_iter]:]}
    #
    #
    #         eval_iter += 1
    #     print(output)
    ...
    def do_eval(self):
        ...
        logging.info("Generating eval data")
        eval_data_changes, dates, _ = self.generate_eval_data(column="high")
        ...
        if 1:
            ...
            for ind, row in enumerate(classes):
                ...
                logging.info(this_original)
                prediction_accuracies_1 += [accuracy_score(y_true_order_1[1:], y_pred_order_1)]
                logging.info("%s, 1st Order, GT: %s, Pred: %s, acc: %s" % (
                    ind, y_true_order_1[1:], y_pred_order_1, prediction_accuracies_1[-1]))
        ...
        prices_close, daily_dates = self.get_prices_eval(column1="close")

        # for ind, k in enumerate(daily_dates):
        #     for r in k:
        #         for l in dates[ind]:
        #             if r > l:
...
class Cnn3ClassWeeklyVolumeLow(FEModel):
    ...
    def generate_train_data(self, column="volume", stats="max"):
        ...
        return train_samples, volume_sample, low_sample
    ...
    def generate_eval_data(self, column="volume", stats="max"):
        ...
        dates = dates_fetch[-self.weeks_to_eval:]

        return train_samples, volume_sample, low_sample, dates
    ...
    def generate_pred_data(self, column="volume", stats="max"):
        ...
        train_samples = values[:, -self.n_steps-1:total_samples_avail]
        train_samples = (train_samples[:, 1:] - train_samples[:, 0:-1]) / train_samples[:, 0:-1]

        train_samples[np.isnan(train_samples)] = 0
        train_samples[np.isinf(train_samples)] = 0

        volume_sample = volume_values[:, -self.n_steps-1:total_samples_avail]
        volume_sample = np.log(1+(volume_sample[:, 1:] - volume_sample[:, 0:-1]) / volume_sample[:, 0:-1])

        volume_sample[np.isnan(volume_sample)] = 0
        volume_sample[np.isinf(volume_sample)] = 0

        low_sample = low_values[:, -self.n_steps-1:total_samples_avail]
        low_sample = (low_sample[:, 1:] - low_sample[:, 0:-1]) / low_sample[:, 0:-1]
        ...
        return train_samples*100, volume_sample, low_values*100, dates
    ...
    def get_pred(self, rows, rows_volume, rows_low):
        # x_train = []

        # for k in range(len(rows) - self.n_steps):
        x_train = [[np.append(np.append(rows, rows_volume), rows_low)]]
    ...
    def do_eval(self):
        ...
        logging.info("Generating eval data")
        eval_data_changes, eval_data_changes_volume, eval_data_changes_low, dates = self.generate_eval_data(column="high")
        logging.info("Generating eval data: Done")
    ...
    def do_pred(self):
        ...
        logging.info("Generating pred data")
        pred_data_changes, pred_data_changes_volume, pred_data_changes_low, dates = self.generate_pred_data(
            column="high")
        logging.info("Generating pred data: Done")
        print(dates)
        ...
        for ind, k in enumerate(pred_data_changes):
            print(pred_data_changes[ind],
                                                  pred_data_changes_volume[ind],
                                                  pred_data_changes_low[ind])
            exit()
            x_train = self.get_pred(pred_data_changes[ind],
                                                  pred_data_changes_volume[ind],
                                                  pred_data_changes_low[ind])
            print(x_train)
            self.load_model("global")
    ...
    def do_eval(self):
        ...
        for ind, row in enumerate(conditional_classes):
            ...
            for k in eval_data_for_this_row[:-1]:
                initial_state = k
                y_pred += [self.predict_next_state(initial_state, transision_matrix[ind])]
            # print("With Vol", y_true)
            y_true_order_1 = [r % self.n_classes for r in y_true]
            y_pred_order_1 = [r % self.n_classes for r in y_pred]
            # print("Without vol", y_true_order_1)
            if class_multiplier[:-1]:
                for k in class_multiplier[:-1]:
                    y_true_order_1 = [r%k for r in y_true_order_1]
                    y_pred_order_1 = [r%k for r in y_pred_order_1]

            this_original = np.ones(eval_price_changes[ind].shape)

            this_original[eval_price_changes[ind] > 0.01] = 2
            this_original[eval_price_changes[ind] <= -0.01] = 0
            # print(order_1_classes_volume[ind][-self.weeks_to_eval:])
            print(this_original[-self.weeks_to_eval:])
...
class Markov2ClassWeeklyCRFVolume(FEModel):
    ...
    def do_eval(self):
        ...
        for ind, row in enumerate(conditional_classes):
            ...
            this_original = eval_price_changes[ind]
            this_original[this_original > -0.01] = 1
            this_original[this_original <= -0.01] = 0
            print(order_1_classes_volume[ind][-self.weeks_to_eval:])
            print(this_original[-self.weeks_to_eval:])
            prediction_accuracies += [accuracy_score(y_true_order_1, y_pred_order_1)]
            logging.info("%s, 1st Order, GT: %s, Pred: %s, acc: %s" % (ind, y_true_order_1, y_pred_order_1, prediction_accuracies[-1]))
    ...
    def predict_next_state(self, initial_state, transition_matrix):
        ...
        randomvalue = np.random.random()
        class_iter = 0
        random_state =  int(trans_cumsum.shape[0]*randomvalue)
        if np.sum(trans_cumsum[initial_state]) == 0:
            print("Not transision possible, inital state: %s, random state: %s" % (initial_state, random_state))
            return random_state
        for pty in trans_cumsum[initial_state]:
            if randomvalue < pty:
                break
...
class Markov2ClassWeekly(FEModel):
    ...
    def do_eval(self):
        ...
        for ind, row in enumerate(classes):
            ...
            this_original = eval_data_changes[ind]
            this_original[this_original > 0] = 1
            this_original[this_original <= 0] = 0
